...
$id_stage_laboratoire=$id;




// Récupération des niveaux
$s_niveaux="SELECT id_niveau, abbreviation, libelle FROM a_niveau 
		WHERE abbreviation IN ('L1','L2','L3','M1','M2','D') ORDER BY id_niveau";
...
$s_ouvertures="SELECT id_niveau, id_specialite, ouvert,id_annee_scolaire FROM l_ouverture_stage
				WHERE id_stage=".$id_stage_laboratoire." AND id_type_stage=0";
...
$creation_tableau=array(
				'Licence' => array('niveaux' => array('L2','L3'),
									'specialites' => array('gd','gf')),
				'Master' => array('niveaux' => array('M1','M2'),
									'specialites' => array('gl','gc','gp','mpt','ge','gs','ssng')),
				);
...
// Création du tableau des ouvetures
	...
	foreach ($creation_tableau['Licence']['specialites'] as $specialite) {
		if ($ouvertures[$id_annee_scolaire][$niveaux[$niveau][0]][$specialites[$specialite][0]]!='') {
			...
			$s_update_ouverture="INSERT INTO l_ouverture_stage (`id_stage`,`id_niveau`,`id_specialite`,`id_annee_scolaire`,`id_type_stage`,`ouvert`) 
				VALUES (".$id_stage_laboratoire.",".$niveaux[$niveau][0].",".$specialites[$specialite][0].",".$id_annee_scolaire.",0,0)";
...
$html.='</table></td>
		<td><table border="0"  class="ouverture_ue"><tr>
		<td>&nbsp;</td>';
